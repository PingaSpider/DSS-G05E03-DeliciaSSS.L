...
<body>
    <form id="form_user" action="{{ route('user.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="container">
            ...
            <input type="text" placeholder="Introduce tu nombre y apellidos" name="nombre" required>
            @error('nombre')
            ...
            <input type="text" placeholder="Introduce tu teléfono" name="telefono" required>
            @error('telefono')
                <div class="error-message">{{ $message }}</div>
            @enderror
